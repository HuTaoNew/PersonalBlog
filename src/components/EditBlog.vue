<template>
  <div id="add-blog">
    ...
    <form action method="post" v-if="!submitted">
      ...
      <button type="button" name="btnAddBlog" @click.prevent="addBlog()">保存博客</button>
    </form>
    <hr>
...
<script>
  export default {
    ...
    methods: {
      addBlog() {
        this.$http
          .put("https://wd3862536764lzhyoq.wilddogio.com/post/" + this.id + '.json', {
    ...
    created() {
      this.$http.get("https://wd3862536764lzhyoq.wilddogio.com/post/" + this.id + '.json')
        .then((response) => {
